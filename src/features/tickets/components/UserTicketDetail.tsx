...
const UserTicketDetail: React.FC = () => {
  // Get ticket ID from params (works for both student and staff routes)
  const routerState = useRouterState();
  const pathname = routerState.location.pathname;
  const ticketId = pathname.split('/').pop();
  
  // Determine the back link based on current route
  const isStaff = pathname.includes('/staff/');
  const backLink = isStaff ? '/staff/tickets' : '/student/tickets';
